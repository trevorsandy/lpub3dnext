    ...
    bool         _split)
{
  switch(_type){
    case TypePli:
      pliMeta = _meta.LPub.pli;
      break;
    case TypeBom:
      bom = true;
      pliMeta = _meta.LPub.bom;
      break;
    case TypeOneToOne:
      oto = true;
      pliMeta = _meta.LPub.oneToOne;
      break;
    case TypeRightWrong:
      rw  = true;
      pliMeta = _meta.LPub.rightWrong;
      break;
    }
  splitBom = _split;

  const int numParts = _csiParts.size();

  for (int i = 0; i < numParts; i++) {
      QString part = _csiParts[i];
      ...
      if (tokens.size() == 15 && tokens[0] == "1") {
          ...
          QString nameKey = QString("%1_%2_%3_%4_%5_%6_%7")
              .arg(key)
              .arg(gui->pageSize(_meta.LPub.page, 0))
              .arg(resolution())
          ...
          if (bom && splitBom){
            ...
            // else if (oto) {   //oneToOne

            // }
            // else if (rw) {    // rightWrong
...
int Pli::sizePli(ConstrainData::Constrain constrain, unsigned height)
{
  if (parts.size() == 0) {
      return 1;
    }
    ...
    ConstrainData &constrainData)
{
  ...
  if (constrainData.type == ConstrainData::ConstrainHeight) {
      int cols;
      int rc;
      rc = placePli(sortedKeys,
                    ...
                    pliHeight);
      if (rc == -2) {
          constrainData.type = ConstrainData::ConstrainArea;
        }
    } else if (constrainData.type == ConstrainData::ConstrainColumns) {
    ...
    } else if (constrainData.type == ConstrainData::ConstrainWidth) {
      ...
      placePli(sortedKeys,10000000,
               ...
               pliWidth,
               pliHeight);
    } else if (constrainData.type == ConstrainData::ConstrainArea) {
      ...
      placePli(sortedKeys,10000000,
               ...
               pliWidth,
               pliHeight);
    } else if (constrainData.type == ConstrainData::ConstrainSquare) {
...
void PliBackgroundItem::resize(QPointF grabbed)
{
  ...
  ConstrainData constrainData;
  constrainData.type = ConstrainData::ConstrainHeight;
