...
void MetaItem::deleteSubModel(Where &topOfStep){
  Rc rc;
  Meta meta;
  Where walk = topOfStep + 1;                                     // advance past STEP meta
  int numLines = gui->subFileSize(topOfStep.modelName);
  scanPastGlobal(topOfStep);
  QRegExp subModelMeta("^\\s*0\\s+!LPUB\\s+.*SUBMODEL");
  for ( ; walk < numLines; walk++) {
      QString line = gui->readLine(walk);
      if (line.contains(subModelMeta)) {
          deleteMeta(walk);
          walk--;                                                 // compensate for deleted line
        } else {
          QStringList tokens;
  ...
  const Where   &toHere)
{
  ...
  if (renderer->useLDViewSCall()) {
      QString monoFile = QDir::currentPath() + "/" + Paths::tmpDir + "/pointerMono.ldr";
      QStringList ldrFiles(monoFile);
      rc = renderer->rotateParts(empty, meta.rotStep, csiParts, monoFile);
      rc = renderer->renderCsi(empty, ldrFiles, empty, meta);
      imageName = QDir::currentPath() + "/" + Paths::assemDir + "/pointerMono.png";
    } else {
      imageName = QDir::currentPath() + "/" + Paths::tmpDir + "/pointerMono.png";
      rc = renderer->renderCsi(empty, csiParts, imageName, meta);
    }
  ...
  emit gui->messageSig(false,QMessageBox::tr("Render momo image for pointer tip failed."));
  return QPointF(0.5,0.5);
}
  ...
  bool    isMirrored)
{
  ...
  if (renderer->useLDViewSCall()) {
      ...
      QString monoFile = QDir::currentPath() + "/" + Paths::tmpDir + "/pointerMono.ldr";
      QStringList ldrFiles(monoFile);
      rc = renderer->rotateParts(addLine, meta.rotStep, csiParts, monoFile);
      rc = renderer->renderCsi(empty, ldrFiles, empty, meta);
      imageName = QDir::currentPath() + "/" + Paths::assemDir + "/pointerMono.png";
    } else {
      imageName = QDir::currentPath() + "/" + Paths::tmpDir + "/pointerMono.png";
      rc = renderer->renderCsi(addLine, csiParts, imageName, meta);
    }
...
void MetaItem::writeRotateStep(QString &value)
{
    ...
    if (multiStep) {
        ...
        if (ok) {
            ...
            emit gui->messageSig(false,"Could not retrieve Step's line number");
            return;
          }
