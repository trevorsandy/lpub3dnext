...
struct pageSizeTypes {
    ...
    float   pageHeightCm;   // not used - for dev reference oinly
    float   pageWidthIn;
    float   pageHeightIn;
} pageSizeTypes[] = {
  ...
  switch (constraint.type) {
    ...
    case ConstrainData::ConstrainHeight:
      string = QString("%1") .arg(constraint.constraint,
                                  4,'f',2);
    break;
    case ConstrainData::ConstrainColumns:
...
void ConstrainGui::typeChange(QString const &type)
{
  ...
  QString string = "";
  if (type == "Area") {
    constraint.type = ConstrainData::ConstrainArea;
  } else if (type == "Square") {
    constraint.type = ConstrainData::ConstrainSquare;
  } else if (type == "Width") {
    string = QString("%1") .arg(constraint.constraint,
                                4,'f',2);
    constraint.type = ConstrainData::ConstrainWidth;
  } else if (type == "Height") {
    string = QString("%1") .arg(constraint.constraint,
                                4,'f',2);
    constraint.type = ConstrainData::ConstrainHeight;
  } else {
    string = QString("%1") .arg(int(constraint.constraint));
    constraint.type = ConstrainData::ConstrainColumns;
  }
