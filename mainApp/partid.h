...
class Pointer;
class PartIDPointerItem;
class PartIDItem;

class PartID : public Steps
{
public:
  Step               *parentStep;
  ...
  QList<PartIDPointerItem *> graphicsPointerList;

  PartIDItem       *background;
  QGraphicsRectItem *underpinnings;
  Where             topPartID,bottomPartID;

  Where &topOfPartID()
  {
    return topPartID;
  }
  Where &bottomOfPartID()
  {
    return bottomPartID;
  }
  void setTopOfPartID(const Where &topOfPartID)
  {
    topPartID = topOfPartID;
  }
  void setBottomOfPartID(const Where &bottomOfPartID)
  {
    bottomPartID = bottomOfPartID;
  }
...
class PartIDItem : public PlacementBackgroundItem
{
public:
  PartID           *partID;
  PartIDMeta        partIDMeta;
  QRect              partIDRect;
  QRect              csiRect;
  AllocMeta         *alloc;
  PageMeta          *page;
...
class PartIDPointerItem : public PointerItem
{
public:
  PartIDPointerItem(
    PartID       *st,
...
private:
   ...
   *
   *
   *  partid size defines the outside edge of the partid.
   ...
   *  intersect the partid are at +- tickness/2.  I'm not
