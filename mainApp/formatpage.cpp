            ...
            case InsertData::InsertBom:
              {
                ...
                if (boms > 1){
                    page->pli.setParts(bomParts,page->meta,TypeBom,true); //Split BOM Parts
                  } else {
                    page->pli.setParts(bomParts,page->meta,TypeBom);
                  }
                bomParts.clear();
                page->pli.sizePli(&page->meta,page->relativeType,false);  // BOM
      ...
      if (page->list.size()) {
          ...
          if (range->relativeType == RangeType) {
              ...
              if (step && step->relativeType == StepType) {
                  ...
                  /* Size the callouts */
                  for (int i = 0; i < step->calloutList.size(); i++) {
                      step->calloutList[i]->sizeIt();
                    }
                  ...
                  for (int i = 0; i < step->calloutList.size(); i++) {
                      ...
                      if (step->calloutList[i]->relativeType == CalloutType) {
                          Callout *callout = dynamic_cast<Callout *>(step->calloutList[i]);
                  ...
                  for (int i = 0; i < step->calloutList.size(); i++) {
                      ...
                      Callout *callout = step->calloutList[i];
